...
void perspective2erect_map_hugin_test();
void fisheye2erect_map_test();
void fisheye2erect_map_test_hugin();
...
void main()
{
	...
	//perspective2erect_map_hugin_test();
  //   fisheye2erect_map_test();
fisheye2erect_map_test_hugin();
...
void perspective2erect_map_hugin_test()
{
	...
	m_maps_y.create(Size(width, height), CV_32FC1);
	Mat output;
	float D[4] = { 0, 0, 0.2, 0.8 };
	imgin2imgout_img.perspective2errect_map_hugin(resource, yaw, pitch, roll, width, height, fov,D, m_maps_x, m_maps_y);
	remap(resource, output, m_maps_x, m_maps_y, CV_INTER_NN, BORDER_CONSTANT);
	cvNamedWindow("Camera Show", 1);
	imshow("Camera Show", output);
	cvWaitKey(0);
...
void fisheye2erect_map_test_hugin()
{
	...
	float corner_f[2];
	float m_size_f[2];
	imgin2imgout_img.fisheye2errect_map_hugin(resource, yaw, pitch, roll, D, width, height, fov, m_maps_x, m_maps_y, corner_f, m_size_f);
	corner.x = corner_f[0]; corner.y = corner_f[1];
	m_size = Size(m_size_f[0], m_size_f[1]);
	remap(resource, output, m_maps_x, m_maps_y, CV_INTER_NN, BORDER_CONSTANT);
	cvNamedWindow("Camera Show", 1);
