...
void imgin2imgout_img::perspective2errect_map_hugin(Mat SrcImage, float yaw, float pitch, float roll, int w_dst, int h_dst, float fov_dst,float D[4], Mat m_maps_x, Mat  m_maps_y)
{
	int row = SrcImage.rows;
	int col = SrcImage.cols;
	//scan from the dst img
	for (int i = 0; i <w_dst; i = i + 1)
	{
		for (int j = 0; j <h_dst; j++)
		{
			...
			float cor_image[2] = { 0, 0 };
			imgotut2imgin.errect2perspective_hugin(yaw, pitch, roll, i, j, w_dst, h_dst, col, row, fov_dst, D ,cor_image);
			u = cor_image[0];
			v = cor_image[1];
...
void imgin2imgout_img::fisheye2errect_map_hugin(Mat SrcImage, float yaw, float pitch, float roll, float D_dst[4], int w_dst, int h_dst, float fov_dst, Mat m_maps_x, Mat  m_maps_y, float corner[2], float m_size[2])
{
	int row = SrcImage.rows;
	int col = SrcImage.cols;
	int x_min = row;
	...
	for (int i = 0; i <w_dst; i = i + 1)
	{
		for (int j = 0; j <h_dst; j++)
		{
			...
			float cor_image[2] = { 0, 0 };
			imgotut2imgin.errect2fisheye_hugin(yaw, pitch, roll, i, j, w_dst, h_dst, col, row, fov_dst, D_dst, cor_image);
			u = cor_image[0];
			v = cor_image[1];
