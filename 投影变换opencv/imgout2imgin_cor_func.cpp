...
void imgout2imgin_cor::errect2perspective_hugin(float yaw, float pitch, float roll, float x_src, float y_src, int w_src, int  h_src, int w_dst, int h_dst, float fov_dst, float D[4], float xy_dst[2])
{
	float cor_wout[3];
	float cor_win[3];
	...
	float yw = cor_win[1];
	float zw = cor_win[2];
	w2i.world2perspective_hugin(xw, yw, zw, fov_dst, h_dst, w_dst, D, xy_dst);
}
...
void imgout2imgin_cor::errect2fisheye_hugin(float yaw, float pitch, float roll, float x_src, float y_src, int w_src, int  h_src, int w_dst, int h_dst, float fov_dst, float D_dst[4], float xy_dst[2])
{
	float cor_wout[3];
	float cor_win[3];
	...
	float yw = cor_win[1];
	float zw = cor_win[2];
	w2i.world2fisheye_hugin(xw, yw, zw, D_dst, fov_dst, h_dst, w_dst, xy_dst);
