...
void wrd2img::world2perspective_hugin(float x_src, float y_src, float z_src, float fov_dst, int h_dst, int w_dst, float D[4], float xy_dst[2])
{
	float f, cx, cy;
	f = (float)w_dst / 2.0f / tan(fov_dst / 2);
	...
	if (z_src > 0)
	{
		...
		float cor_c[3][1] = { { x_src }, { y_src }, { z_src } };
		float r = sqrt(int(x_src) ^ 2 + int(y_src) ^ 2);
		float x_src_dis = (D[0]*r*r*r + D[1]*r*r + D[2]*r + D[3])*x_src;
		float y_src_dis = (D[0] * r*r*r + D[1] * r*r + D[2] * r + D[3])*y_src;
		float cor_c_dis[3][1] = { { x_src_dis }, { y_src_dis }, { z_src } };
		float result_z[3];

		matrix_mul(k, cor_c_dis, result_z);
		xy_dst[0] = result_z[0];
		xy_dst[1] = result_z[1];
	}
	else
...
void wrd2img::world2fisheye_hugin(float x_src, float y_src, float z_src, float D_dst[4], float fov_dst, int h_dst, int w_dst, float xy_dst[2])
{
	//float f = sqrt(w_dst*w_dst + h_dst*h_dst) / fov_dst;
	float f = (w_dst) / fov_dst;
	float cx = w_dst / 2;
	...
	if (z_src > 0)
	{
		...
		else
		{
			...
			float theta_d = theta * (D_dst[0]*theta3+
				D_dst[1] * theta2 +
				D_dst[2] * theta +
				D_dst[3] );
			x_src = x_src / r * theta_d;
