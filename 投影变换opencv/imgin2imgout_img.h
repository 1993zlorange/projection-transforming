...
public:
	...
	*/
	...
	/*
	D_dst[4]={a,b,c,d} && a+b+c+d=1
	*/
	void fisheye2errect_map_hugin(Mat SrcImage, float yaw, float pitch, float roll, float D_dst[4], int w_dst, int h_dst, float fov_dst, Mat m_maps_x, Mat  m_maps_y, float corner[2], float m_size[2]);

	/*
	D[4]={a,b,c,d} && a+b+c+d=1
	*/
	void perspective2errect_map_hugin(Mat SrcImage, float yaw, float pitch, float roll, int w_dst, int h_dst, float fov_dst, float D[4], Mat m_maps_x, Mat  m_maps_y);
	//!erect to perspective demo
